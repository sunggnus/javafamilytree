...
public class ForceXPosition {
	...
	/**
	 * this algorithm uses a force based approach for sorting the x position of
	 * the graph this algorithm should be called after the y sorting is finished
	 * 
	 * @param top
	 */
		...
		for (int i = 1; i < maxgen + 1; i++) {
			...
			for (Person person : inTree) {
				if (person.getGeneration() == i) {
					...
					if (!added) {
						List<Person> partners = Utils
								.getPartnerConnections(person);
						partners.add(person);
