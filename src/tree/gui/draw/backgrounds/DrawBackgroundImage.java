...
public class DrawBackgroundImage extends Background {
	...
	public DrawBackgroundImage(DrawImage basic)
			throws InvalidDrawImageException {
		if (basic == null) {
	...
	public void paintPicture(Graphics g, int x, int y) {
		...
		this.eastSouth.paintPicture(g, x + this.westNorth.getImageWidth(), y
				+ this.westNorth.getImageHeight());
	}
	...
	public void paintBackground(Graphics2D g, int xPos, int yPos) {
		...
		case MODE_SCALED:
			...
			g.drawImage(image, (int) (xPos * scale), (int) (yPos * scale),
					(int) (scale * imWidth), (int) (scale * imHeight), null);
			break;
		case MODE_FIT:
