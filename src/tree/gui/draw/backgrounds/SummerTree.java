...
public class SummerTree extends Background {
	...
	public void bufferTree() {
		tree = new BufferedImage(this.getWidth(), this.getHeight(),
		...
		this.paintTree(g);
		System.out.println("new tree painted width " + this.getWidth()
	...
	protected Color nextRightColor(Color oldColor) {
		return new Color(
				...
				(int) (0.9 * oldColor.getGreen() + 0.1 * Color.YELLOW
						.getGreen()),
	...
	protected void drawBranch(double xStart, double yStart, double xEnd,
			double yEnd, double thickness, Color color) {
	...
	protected void drawTree() {
		...
		if (random) {
			drawTreeRandom(this.getWidth() / 2,
					this.getHeight() + this.getHeight() / 10, -Math.PI / 2.0,
					lengthX / 5.1, this.getHeight() / 4, lengthX / 32.0,
					new Color(155, 100, 60));
		} else {
			drawTreeRec(this.getWidth() / 2,
					this.getHeight() + this.getHeight() / 11, -Math.PI / 2.0,
					lengthX / 5.1, this.getHeight() / 4, lengthX / 32.0,
					new Color(155, 100, 60));
		}
		// drawTreeRec(this.getWidth() / 2, this.getHeight(),
		// -Math.PI / 2.0, lengthX/5, this.getHeight()/4, lengthX/32.0, new
		// Color(155, 100, 60));
	...
	protected void drawTreeRec(double x, double y, double alpha,
		...
		drawTreeRec(x + distanceX, y + distanceY, alpha - 0.5,
				lengthX * 9 / 12, lengthY * 27 / 35, thickness * 9 / 10,
				nextLeftColor(color));
		drawTreeRec(x + distanceX, y + distanceY, alpha + 0.6,
				lengthX * 9 / 12, lengthY * 27 / 35, thickness * 9 / 10,
				nextRightColor(color));
	}
	...
	protected void drawTreeRandom(double x, double y, double alpha,
		...
		drawTreeRec(x + distanceX, y + distanceY, alpha - 0.5, lengthX * rnd2,
				lengthY * rnd2, thickness * 9 / 10, nextLeftColor(color));
		drawTreeRec(x + distanceX, y + distanceY, alpha + 0.6, lengthX * rnd3,
				lengthY * rnd3, thickness * 9 / 10, nextRightColor(color));
