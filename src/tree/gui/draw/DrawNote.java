...
public class DrawNote extends AbstractDraw{
	...
	private int stringHeight;
		...
		public void draw(Graphics2D g,   int widthUnit, int heightUnit, double scaling, boolean drawX){
			...
			g.setFont(new Font(orgFont.getFontName(),orgFont.getStyle(),this.note.getFontSize()));
		...
		protected void calculateBounds(int widthUnit, int heightUnit, double scaling, Graphics2D g){
		...
		@Override
		public void calculateBounds(int widthUnit, int heightUnit,
				double scaling) {
			...
			int y = (int) (this.note.getY() * scaling * heightUnit * 1.3)+
					(int)Math.ceil(this.note.getSmoothY()*heightUnit * scaling)-
					(int) (scaling * stringHeight);
			
			this.setMinX(x);
			this.setMinY(y);
			this.setMaxY(y+(int) Math.ceil(this.note.getComments().size()*stringHeight * scaling ));
