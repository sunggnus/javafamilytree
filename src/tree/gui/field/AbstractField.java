...
public abstract class AbstractField extends JPanel {
	...
	final static public int DEFAULT_WIDTH = EMPTY_TEXT_FIELD_WIDTH
			+ DEFAULT_LABEL_WIDTH;
	...
	/**
	 * use this constructor only for the gaining of the GroupSize feature and
	 * override the increaseLabelWidth in the sub class
	 */
	...
	protected AbstractField(String name, int labelWidth, JComponent field,
		...
		this.field.setMinimumSize(new Dimension(fieldWidth, (int) this.field
				.getPreferredSize().getHeight()));
		this.label.setMinimumSize(new Dimension(labelWidth, (int) this.label
				.getPreferredSize().getHeight()));
	...
	private void refreshMinDim(double fieldWidth, double labelWidth) {
		...
		minDim.setSize(fieldWidth + labelWidth + 10, (this.field
				.getPreferredSize().getHeight() > this.label.getPreferredSize()
				.getHeight()) ? this.field.getPreferredSize().getHeight()
				: this.label.getPreferredSize().getHeight());
	...
	public void increaseLabelWidth(int width) {
		Dimension newDim = new Dimension(width, (int) this.getLabel()
				.getPreferredSize().getHeight());
		...
		this.refreshMinDim(
				this.getFieldComponent().getMinimumSize().getWidth(), this
						.getLabel().getMinimumSize().getWidth());
	...
	public void triggerResizeEvent() {
		if (this.size != null) {
			this.size.processEvent((int) this.getLabel().getPreferredSize()
					.getWidth());
		}
	}
