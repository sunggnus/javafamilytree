...
public class OptionDialog extends JDialog {
	...
	private void constructOptions() {
		...
		DropDownField<OptionList> mode = new DropDownField<OptionList>(Main
				.getTranslator().getTranslation("displayMode",
						Translator.LanguageFile.OPTION_DIALOG), width);

		DropDownField<OptionList> lineDrawMode = new DropDownField<OptionList>(
				Main.getTranslator().getTranslation("lineMode",
						Translator.LanguageFile.OPTION_DIALOG), width);

		DropDownField<OptionList> backgroundMode = new DropDownField<OptionList>(
				Main.getTranslator().getTranslation("backgroundMode",
						Translator.LanguageFile.OPTION_DIALOG), width);

		final ModifiedJSlider xBackgroundPosition = new ModifiedJSlider(Main
				.getTranslator().getTranslation("xSlider",
						Translator.LanguageFile.OPTION_DIALOG), width);
		final ModifiedJSlider yBackgroundPosition = new ModifiedJSlider(Main
				.getTranslator().getTranslation("ySlider",
						Translator.LanguageFile.OPTION_DIALOG), width);
		initSliders(xBackgroundPosition, yBackgroundPosition);

		DropDownField<OptionList> lineBreakMode = new DropDownField<OptionList>(
				Main.getTranslator().getTranslation("lineBreakMode",
						Translator.LanguageFile.OPTION_DIALOG), width);
		// text ist still lineBreakMode
		DropDownField<OptionList> dataPositioningMode = new DropDownField<OptionList>(
				"", width);

		DropDownField<OptionList> mouseMode = new DropDownField<OptionList>(
				Main.getTranslator().getTranslation("mouseMode",
						Translator.LanguageFile.OPTION_DIALOG), width);

		DropDownField<OptionList> keyboardMode = new DropDownField<OptionList>(
				Main.getTranslator().getTranslation("keyboardMode",
						Translator.LanguageFile.OPTION_DIALOG), width);
		DropDownField<OptionList> lookAndFeelMode = new DropDownField<OptionList>(
				Main.getTranslator().getTranslation("lookAndFeelMode",
						Translator.LanguageFile.OPTION_DIALOG), width);

		DropDownField<OptionList> yPositioningMode = new DropDownField<OptionList>(
				Main.getTranslator().getTranslation("ypositioning",
						Translator.LanguageFile.OPTION_DIALOG), width);

		DropDownField<OptionList> treeOrderingMode = new DropDownField<OptionList>(
				Main.getTranslator().getTranslation("treeOrderingMode",
						Translator.LanguageFile.OPTION_DIALOG), width);

		final DropDownField<LookAndFeelInfo> additionalLookAndFeel = new DropDownField<LookAndFeelInfo>(
				"", width);
		...
		additionalLookAndFeel.setVisible(Config.LOOK_AND_FEEL_MODE
				.equals(OptionList.ADDITIONAL_LOOK_AND_FEEL));
		...
		treeOrderingMode.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				if (e.getStateChange() == ItemEvent.SELECTED
						&& e.getItem() instanceof OptionList) {
		...
		mouseMode.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent arg0) {
				if (arg0.getStateChange() == ItemEvent.SELECTED
						&& arg0.getItem() instanceof OptionList) {
		...
		lookAndFeelMode.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent arg0) {
				if (arg0.getStateChange() == ItemEvent.SELECTED
					...
					additionalLookAndFeel.setVisible(item
							.equals(OptionList.ADDITIONAL_LOOK_AND_FEEL));
					remove(panel);
					constructOptions();
				}
		...
		additionalLookAndFeel.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent arg0) {
				if (arg0.getStateChange() == ItemEvent.SELECTED
						&& arg0.getItem() instanceof LookAndFeelInfo) {
		...
		final EntryField unitWidth = new EntryField(Main.getTranslator()
				.getTranslation("defaultWidth",
						Translator.LanguageFile.OPTION_DIALOG), width);
		unitWidth.setContent(Config.PERSON_WIDTH);
		final EntryField unitHeight = new EntryField(Main.getTranslator()
				.getTranslation("defaultHeight",
						Translator.LanguageFile.OPTION_DIALOG), width);
		...
		unitWidth.addDocumentListener(new DocumentListener() {
			...
			public void changedUpdate(DocumentEvent arg0) {
				try {
					Config.PERSON_WIDTH = Integer.parseInt(unitWidth
							.getContent());
		...
		unitHeight.addDocumentListener(new DocumentListener() {
			...
			public void changedUpdate(DocumentEvent arg0) {
				try {
					Config.PERSON_HEIGHT = Integer.parseInt(unitHeight
							.getContent());
				} catch (NumberFormatException e) {
		...
		ord.add(new JLabel(Main.getTranslator().getTranslation("orderingHead",
				Translator.LanguageFile.OPTION_DIALOG)), BorderLayout.NORTH);
		ord.add(ordering);
	...
	static private void initSliders(ModifiedJSlider xMSlider,
			ModifiedJSlider yMSlider) {
		...
		if (BackgroundFactory.getBufferedBackground() != null
				...
				&& Config.BACKGROUND_MODE
						.equals(OptionList.DRAW_LOADED_BACKGROUND)) {
			DrawBackgroundImage background = (DrawBackgroundImage) BackgroundFactory
					.getBufferedBackground();
	...
	static private ChangeListener createChangeListener(final char a,
			final DrawBackgroundImage background) {
