...
public class PersonEditLine extends AbstractPersonLine {
	...
	public PersonEditLine(final Person person, final AbstractOverview view) {
		...
		delete = new JButton(Main.getTranslator().getTranslation("remove",
				Translator.LanguageFile.OVERVIEW_DIALOG));
		edit = new JButton(Main.getTranslator().getTranslation("edit",
				Translator.LanguageFile.OVERVIEW_DIALOG));
		visible = new JButton(Main.getTranslator().getTranslation(
				"makeVisible", Translator.LanguageFile.OVERVIEW_DIALOG));
		focus = new JButton(Main.getTranslator().getTranslation("focus",
				Translator.LanguageFile.OVERVIEW_DIALOG));

		if (person.isVisible()) {
			visible.setText(Main.getTranslator().getTranslation(
					"makeInvisible", Translator.LanguageFile.OVERVIEW_DIALOG));
		}
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (Main.getMainNode().getPerson() == person) {
					if (Main.getMainNode().getPersons().size() > 0) {
						Main.getMainNode().setPerson(
								Main.getMainNode().getPersons().get(0));
					} else {
		...
		focus.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				int y = (int) (person.getGeneration() * scaling * heightUnit
						* 1.3 + DrawPerson.DEFAULT_MARGING);
