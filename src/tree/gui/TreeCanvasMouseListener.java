...
/**
 * TreeCanvasMouseListener is a MouseListener and a MouseMotionListener which
 * listens on MouseEvents happening within the TreeCanvas. This
 * TreeCanvasMouseListener makes it possible to drag and drop person nodes
 * within the canvas
 * 
 * @author Stefan
 *
 */
public class TreeCanvasMouseListener implements MouseListener,
		MouseMotionListener {
	...
	/**
	 * constructs a new {@link TreeCanvasMouseListener} a
	 * {@link TreeCanvasMouseListener} should be added only by one
	 * {@link TreeCanvas}!
	 * 
	 * @param canvas
	 *            the TreeCanvas by which this MouseListener is added
	 */
	...
	/**
	 * if move is true the listener listens on new MousePositions and move
	 * marked persons correspondingly
	 */
	private boolean move = false;

	/**
	 * if mark is true a border can be created an everything within will be
	 * marked
	 */
			...
			if (draw.containsCoordinates(x, y)) {
				if (arg0.getButton() == mouseButton1) {
					if (draw instanceof DrawPerson) {
						new EditPersonDialog(
								((DrawPerson) draw).getDrawPerson());
						break;
					}
	...
	public void mouseReleased(MouseEvent arg0) {
		...
		if (mark
				&& (Math.abs(pressedX - lastX) + Math.abs(pressedY - lastY) > 30)) {
			int x = arg0.getX();
			int y = arg0.getY();
			int upperX = pressedX;
			int lowerX = x;
			...
			for (AbstractDraw draw : canvas.getToDraw()) {
				...
				if (drawXMin < upperX && drawXMax > lowerX && drawYMin < upperY
						&& drawYMax > lowerY) {
					draw.setMarked(true);
				} else {
	...
	public void mouseMoved(MouseEvent arg0) {
		...
		if (move) {
			...
			int difX = (int) (2 * (x - pressedX) / ((double) canvas
					.getWidthUnit() * canvas.getScaling() * 0.91));
			int difY = (int) ((y - pressedY) / ((double) canvas.getHeightUnit()
					* canvas.getScaling() * 1.15));
			...
			for (AbstractDraw draw : canvas.getToDraw()) {
				if (draw.isMarked()) {
					...
					draw.setRelativeYCoordinate(draw.getRelativeYCoordinate()
							+ difY);
