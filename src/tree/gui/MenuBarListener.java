...
public enum MenuBarListener implements ActionListener {
	...
	/**
	 * loads an image and mirrors it on the x and y axis then draw it on the
	 * background of the tree multiplying it so it fills out the whole area
	 */
	...
	EXPORT_BACKGROUND_AS_JPEG() {
		...
		public void actionPerformed(ActionEvent arg0) {
			EXPORT_BACKGROUND_PICTURE.actionPerformed(new ActionEvent(arg0
					.getSource(), arg0.getID(), "jpeg"));
	...
	/**
	 * opens an overview window with every note it is possible to search for
	 * certain notes there, to open an edit frame for each note and to delete
	 * them
	 */
	NOTE_OVERVIEW() {
		@Override
		public void actionPerformed(ActionEvent arg0) {
			new NoteOverview(Main.getMainNode().getNotesReference(),
					new DefaultFilter(), new EditLineFactory());
		}
	},
	/**
	 * opens an overview window with every person it is possible to search for
	 * certain persons there, to open an edit frame for each person and to
	 * delete them
	 */
	PERSON_OVERVIEW() {
		@Override
		public void actionPerformed(ActionEvent arg0) {
			new PersonOverview(Main.getMainNode().getPersonsReference(),
					new DefaultFilter(), new EditLineFactory(), true);
		}
	},
	/**
	 * change the draw x-y-position option if true the grid net x and y position
	 * of each person is written within the person frame on the tree so it is
	 * easier to edit the tree
	 */
	DRAW_X_Y_POSITION() {
		@Override
		public void actionPerformed(ActionEvent arg0) {
