...
public final class Config {
	...
	static public final String LINE_SEPARATOR = System.getProperties()
	...
	/**
	 * determines if given name and family name are written down in the same
	 * line or not
	 */
	...
	/**
	 * contains the path to the default folder you will see when you open a file
	 * chooser
	 * 
	 */
	...
	/**
	 * true if the model should try to calculate the x position of the persons
	 * automatically
	 */
	...
	static public void readConfigIni() {
		...
		if (config.exists()) {
				...
				while (in.ready()) {
					try {
						...
						if (line.contains("defaultpath=")) {
							String defaultpath = line.replaceAll(
									"defaultpath=", "");
							...
							try {
								PERSON_WIDTH = Integer.parseInt(line
										.replaceAll("personwidth=", "").trim());
							} catch (NumberFormatException e) {
								PERSON_WIDTH = width;
							}
						} else if (line.contains("additionalLookAndFeel=")) {
							ADDITIONAL_LOOK_AND_FEEL = line.replace(
									"additionalLookAndFeel=", "").trim();
						} else if (line.contains("ordering=")) {
							String[] order = line.replace("ordering=", "")
									.trim().split(";");
							if (order.length > ORDERING.length) {
								javax.swing.JOptionPane.showMessageDialog(null,
										"Warnung: Folgende config.ini Zeile fehlerhaft (zuviele Argumente): "
												+ line);
							...
							for (int i = 0; i < order.length
									&& i < ORDERING.length; i++) {
								try {
									DrawPerson.Ordering add = DrawPerson.Ordering
											.valueOf(order[i]);
									ORDERING[i] = add;
									...
									javax.swing.JOptionPane.showMessageDialog(
											null,
											"Warnung: Folgende config.ini Zeile fehlerhaft: "
													+ line);
								}
							}
						} else {
							ORIENTATION_MODE = readOption(line,
									ORIENTATION_MODE.getConfigName(),
									ORIENTATION_MODE);
							CONNECTION_MODE = readOption(line,
									CONNECTION_MODE.getConfigName(),
									CONNECTION_MODE);
							BACKGROUND_MODE = readOption(line,
									BACKGROUND_MODE.getConfigName(),
									BACKGROUND_MODE);
							LINE_BREAK_MODE = readOption(line,
									LINE_BREAK_MODE.getConfigName(),
									LINE_BREAK_MODE);
							DATA_POSITIONING_MODE = readOption(line,
									DATA_POSITIONING_MODE.getConfigName(),
									DATA_POSITIONING_MODE);
							MOUSE_MODE = readOption(line,
									MOUSE_MODE.getConfigName(), MOUSE_MODE);
							KEYBOARD_MODE = readOption(line,
									KEYBOARD_MODE.getConfigName(),
									KEYBOARD_MODE);
							LOOK_AND_FEEL_MODE = readOption(line,
									LOOK_AND_FEEL_MODE.getConfigName(),
									LOOK_AND_FEEL_MODE);
							Y_POSITIONING_MODE = readOption(line,
									Y_POSITIONING_MODE.getConfigName(),
									Y_POSITIONING_MODE);
							TREE_ORDERING_MODE = readOption(line,
									TREE_ORDERING_MODE.getConfigName(),
									TREE_ORDERING_MODE);
						}
			...
			javax.swing.JOptionPane
					.showMessageDialog(
							null,
							"Warnung: Keine config.ini gefunden, es werden die Default Parameter verwendet.");
		}
	...
	static private OptionList readOption(String line, String pattern,
			OptionList org) {
		...
		try {
			class OptionWriter extends BufferedWriter {
				...
				public void write(OptionList option) throws IOException {
					write(option.getConfigName() + option.name()
							+ LINE_SEPARATOR);
				}
			...
			out.write("additionalLookAndFeel=" + ADDITIONAL_LOOK_AND_FEEL
					+ LINE_SEPARATOR);
