...
public class Utils {
	...
	static public GregorianCalendar stringToCalendar(String str){
		...
		if(!str.startsWith(getMonthName(month))){
			...
			int year = Integer.parseInt(str.replace(getMonthName(month), "").trim());
	...
	static private Person getTopLevelPerson(List<Person> persons){
		...
		class PersonWrap{
			...
			public PersonWrap getFather(){
				PersonWrap result = new PersonWrap();
				result.person = person.getFather();
				result.deep = deep + 1;
				return result;
			}
			
			public PersonWrap getMother(){
				PersonWrap result = new PersonWrap();
				result.person = person.getFather();
				result.deep = deep + 1;
				return result;
			...
			private PersonWrap getTopLevelPerson(PersonWrap person){
				PersonWrap result = null;
		
				
				if(person.getFather() != null){
					result = getTopLevelPerson(person.getFather());
				}
				if(person.getMother() != null){
					PersonWrap mother = getTopLevelPerson(person.getMother());
					if(result != null && mother != null && result.deep < mother.deep){
						result = mother;
					}
		...
		PersonWrap best = null;
		for(Person person : persons){
			PersonWrap wrap = new PersonWrap();
			
			wrap.person = person;
			wrap.deep = 0;
			wrap = wrap.getTopLevelPerson(wrap);
			if(best == null || wrap.deep > best.deep)
				best = wrap;
