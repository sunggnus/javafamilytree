...
public abstract class AbstractLine extends JPanel{
	...
	public void setMainSize(int width,int height){
		...
		for(Component comp : this.getComponents()){
			...
			comp.setMaximumSize(new Dimension(500,(int)comp.getPreferredSize().getHeight()));
			}
			else{
				fillerCounter++;
			}
		...
		this.setMinimumSize(new Dimension(width*multi+fillerCounter*5,height));
		GUIUtils.normalizeSize(this);
		this.setMaximumSize(new Dimension((int) this.getMaximumSize().getWidth(),(int) this.getPreferredSize().getHeight()+5));
		
	}
	
	@Override
	public Component add(Component comp){
		super.add(new Box.Filler(new Dimension(5,5), new Dimension(5,5), new Dimension(5,5)));
		super.add(comp);
